...
function getMenu(){
        ...
        return new Promise((resolve, reject) => {
            resolve(fetch("https://free-food-menus-api-production.up.railway.app/burgers").then((response)=>{
                ...
                <div><img src="${products.img}"/></div><div class ="side">Name : ${products.name}</div><div class ="side">Dsc : ${products.dsc}</div><div class ="side">Price : ${products.price}</div><div class ="side">Rate : ${products.rate}</div><div class ="side">Country : ${products.country}</div></div>
                `
...
function takeOrder() {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        //   { name: 'Burger D', price: 8 },
        //   { name: 'Burger E', price: 11 },
        // ];
        const burgers = [
            {
        ...
        // Add burgers to an object
        const order = { burgers: selectedBurgers };
        resolve(console.log("Order Items : ", order));
        // console.log(order)
      }, 2500);
    });
  }
  ...
  function orderPrep() {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        resolve(console.log("Order Status : ",orderStatus));
      }, 1500);
    });
  }
  ...
  function payOrder() {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        resolve(console.log("Payment Status : ",orderStatus));
      }, 1000);
    });
  }
...
//  payOrder()
  ...
  function thankyouFnc() {
    return new Promise((resolve, reject) => {
     ...
     resolve(alert('Thank you for your order!'));  
    })
...
//  thankyouFnc()
  ...
  async function done(){
    let zero = await getMenu()
    let one = await takeOrder()
    let two = await orderPrep()
    let three = await payOrder()
    let four = await thankyouFnc()
